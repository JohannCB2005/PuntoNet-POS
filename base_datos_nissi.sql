...
-- =============================================================================
-- ENTREGAS DE MÓDULOS ESCOLARES (back-office)
-- Padrón de alumnos, comprobantes de pago (cubicol), conciliación, promociones
-- "Módulo del Bimestre" y entregas físicas de módulos con stock vinculado.
-- Misma estructura que migrations/add_modulos_entregas.sql. Un solo colegio.
-- =============================================================================
...
-- Registro de importaciones (padrón o comprobantes) + resumen.
...
-- Comprobantes de pago de pensión. id_alumno NULL = bandeja de conciliación.
CREATE TABLE `pagos` (
  `id_pago` int(11) NOT NULL AUTO_INCREMENT,
...
-- Historial de conciliación manual (asignar/descartar/desasignar).
...
-- Promociones: criterios de elegibilidad ("Módulo del Bimestre a cambio de pensión").
...
-- Detalle de productos por (nivel, grado) de cada promoción.
...
-- Entregas: quién recibió el módulo de una promoción.
...
-- Detalle de una entrega: los productos realmente entregados (foto).
